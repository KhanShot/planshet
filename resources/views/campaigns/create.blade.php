...
@section('content')
    ...
    <div>
        <form method="post"  class="d-flex dropzone" action="{{ route('advertisers.store') }}">
        <div class="col-md-5">
            <div class="d-flex align-items-center mr-5 mt-3 mb-4">
                ...
                <div class="form-group">
                    <select class="form-select border-radius-20 pl-4 pr-4 pt-2 pb-2" name="advertiser">
        ...
        <div class="col-md-7 w-100 back-image">
            <input type="file" accept="video/*" class="my-pond h-100" name="video"/>
        </div>
...
<script>
    ...
    document.getElementById("start_date").min = new Date().addDays(1).toISOString().split("T")[0];
    document.getElementById("end_date").min = new Date().addDays(2).toISOString().split("T")[0];
</script>
@endsection
